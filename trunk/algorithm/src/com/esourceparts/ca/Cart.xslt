...
<xsl:include href="HeaderFooter.xslt" />
	...
	<xsl:template name="Body">
      ...
      <xsl:if test="/OnlineStore/BuySafeRollover"><xsl:value-of select="/OnlineStore/BuySafeRollover/RolloverScript" disable-output-escaping="yes"/></xsl:if>
                  ...
                  <tr>
                    ...
                    <td class="cart_subtotal_price" valign="center">
                         <xsl:choose>
							...
							<xsl:otherwise><xsl:value-of select="/OnlineStore/Cart/Subtotal/Money/Formated" disable-output-escaping="yes"/></xsl:otherwise>
						</xsl:choose>
                    </td>
                    <td>
                      <table cellpadding="0" cellspacing="0" border="0">
                        <tr>
                          <td class="update_btn"><input type="submit" value="Update Cart" title="Update Cart" class="button" /></td>
                        </tr>
                      </table>
                    </td>
                  </tr>
...
<xsl:value-of select="Name"/><xsl:if test="position()!=last()">;</xsl:if>
                      ...
                      <td class="cart_6"><xsl:call-template name="RemoveButtonGlobal"/>
...
<xsl:call-template   name="BuySafeShoppingCartGlobal" />                     
            ...
            <tr>
              <td align="center">
                <input type="button" title="Continue Shopping" value="Continue Shopping"
							class="button" onclick="window.location.replace('/Catalog.aspx')" id="btn_shoppingcart_continueshopping" /> 
